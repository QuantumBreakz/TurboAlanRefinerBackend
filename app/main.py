...
# MongoDB database client for persistent storage
# from app.core.database import init_database, upsert_job, get_job, list_jobs
from app.core.file_versions import file_version_manager
from app.core.strategy_feedback import strategy_feedback_manager, StrategyFeedback
...
from app.core.state import (
    ...
    safe_jobs_snapshot_set, safe_jobs_snapshot_get,
    safe_active_tasks_set, safe_active_tasks_del,
    safe_upsert_job, safe_get_job,
    rate_limit_lock, shared_state_lock,
    MAX_UPLOADED_FILES, MAX_JOBS_SNAPSHOT, MAX_ACTIVE_TASKS,
    RATE_LIMIT_WINDOW, RATE_LIMIT_MAX_REQUESTS
)
from app.api.routes.refine import router as refine_router
from app.core.websocket_progress import router as websocket_router
...
# File size limit - configurable via environment variable (default: 50MB for production)
MAX_FILE_SIZE = int(os.getenv("MAX_FILE_SIZE_MB", "50")) * 1024 * 1024  # Default: 50MB
MAX_REQUEST_TIMEOUT = 1800  # 30 minutes for large file processing
...
# APIError, _make_json_safe, create_error_response moved to app.core.errors

def handle_api_error(func):
    """Deprecated: Prefer global exception handlers to avoid FastAPI 422 on wrappers."""
    return func

from app.core.diff_utils import generate_diff, format_change_for_api, format_statistics_for_api
from app.services.conversation_refiner import ConversationalRefiner
from app.services.pipeline_service import RefinementPipeline  # ⭐ REAL PIPELINE
from app.core.language_model import OpenAIModel, analytics_store  # ⭐ REAL MODEL
from app.core.storage import LocalSink, DriveSink  # ⭐ REAL STORAGE
from app.services.Andy_speech import RefinementMemory, refine_with_feedback  # ⭐ REAL MEMORY + FEEDBACK
from app.core.prompt_schema import ADVANCED_COMMANDS  # ⭐ REAL SCHEMA
...
async def lifespan(app: FastAPI):
    ...
    try:
        from app.core.env_validation import validate_required_env_vars, get_env_summary
...
from app.core.exceptions import RefinerException, ValidationError as RefinerValidationError
...
# Register routers
app.include_router(websocket_router)  # WebSocket endpoints for real-time updates
app.include_router(auth_router)  # Password reset endpoints
...
async def request_timeout_middleware(request, call_next):
    try:
        # Exempt SSE endpoints from timeout (they handle their own timeouts via streaming)
        if request.url.path.startswith("/refine/run") or request.url.path.startswith("/ws/"):
            # SSE endpoints can run longer - let them handle their own timeouts
            return await call_next(request)
        
        # Set a timeout for other request processing
...
# Globals, State, and MemoryManager moved to app.core.dependencies, app.core.state, and app.core.memory_manager
    ...
    try:
        if not database_initialized:
            ...
            if mongodb_db.is_connected():
                test_jobs = mongodb_db.get_jobs(1)
                health_status["checks"]["database"] = "ok"
            else:
                health_status["checks"]["database"] = "MongoDB not connected"
                all_checks_passed = False
    except Exception as e:
...
async def job_status(job_id: str) -> Dict[str, Any]:
    ...
    # First check snapshot (most up-to-date, in-memory)
    snap = jobs_snapshot.get(job_id)
    if snap:
        return {"jobId": job_id, **snap} if isinstance(snap, dict) else {"jobId": job_id, "status": snap.get("status", "unknown") if isinstance(snap, dict) else "unknown"}
    
    # Then check MongoDB (persistent storage)
    if mongodb_db.is_connected():
        try:
            ...
            # Log but don't fail - fall through to in-memory
            pass
    ...
    # Return unknown status if nothing found
    return {"jobId": job_id, "status": "unknown", "message": "Job not found in any storage"}

@app.get("/jobs")
async def jobs_list(user_id: Optional[str] = None) -> Dict[str, Any]:
    """List jobs from MongoDB"""
    return {"jobs": mongodb_db.get_jobs(100, user_id)}
...
async def memory_endpoint(request: MemoryRequest):
    try:
        ...
        from app.core.logger import log_exception; log_exception("MEMORY_API_ERROR", e)
...
async def get_memory_stats(user_id: str):
    ...
    try:
        ...
        from app.core.logger import log_exception; log_exception("MEMORY_STATS_ERROR", e)
...
async def export_memory(user_id: str):
    ...
    try:
        ...
        from app.core.logger import log_exception; log_exception("MEMORY_EXPORT_ERROR", e)
...
async def analytics_jobs(user_id: Optional[str] = None) -> Dict[str, Any]:
    return {"jobs": mongodb_db.get_jobs(200, user_id)}

@app.get("/logs")
async def read_logs(lines: int = 200, level: Optional[str] = None) -> Dict[str, Any]:
    """Read system logs from MongoDB"""
    if not mongodb_db.is_connected():
        return {"logs": [], "lines": []}
    
    logs = mongodb_db.get_system_logs(lines, level)
    
    # Format for frontend compatibility - return both structured and formatted
    structured_logs = []
    formatted_lines = []
    
    for log in logs:
        # Get timestamp - already converted to ISO string by get_system_logs
        ...
        # Structured format for frontend (matching LogEntry interface)
        ...
        # Formatted line for backward compatibility
        formatted_lines.append(f"{ts_str} - {lvl} - {msg}")
...
async def list_drive_files(request: DriveFileRequest = Depends()):
    ...
    try:
        # Try to get credentials (from env var or file)
        creds = get_google_credentials()
        if not creds:
...
async def get_analytics_summary(user_id: Optional[str] = None):
    ...
    try:
        logger.debug(f"Analytics endpoint called: user_id={user_id}")
        
        # Use ONLY MongoDB for analytics - no in-memory merging
        if not mongodb_db.is_connected():
            logger.warning("MongoDB not connected, returning empty analytics")
            return {
                "openai": {
                    "total_requests": 0,
                    "total_tokens_in": 0,
                ...
                "jobs": {
                    ...
                    "performanceMetrics": {
                        ...
                        "avgTensionPercent": 0.0,
                        "avgProcessingTime": 0.0,
                        "avgRiskReduction": 0.0
                    },
                    "recentActivity": []
                }
            }
        
        # Get MongoDB analytics ONLY (no in-memory merge)
        mongodb_openai = mongodb_db.get_aggregate_analytics(user_id)
        mongodb_schema = mongodb_db.get_schema_usage_stats(user_id)
        mongodb_last_24h = mongodb_db.get_last_24h_analytics(user_id)
        
        # Use MongoDB data exclusively
        merged_openai = {
            "total_requests": mongodb_openai.get("total_requests", 0),
            "total_tokens_in": mongodb_openai.get("total_tokens_in", 0),
            "total_tokens_out": mongodb_openai.get("total_tokens_out", 0),
            "total_cost": mongodb_openai.get("total_cost", 0.0),
            "current_model": mongodb_openai.get("current_model", "gpt-4"),
            "last_24h": mongodb_last_24h
        }
        
        # Use MongoDB schema data exclusively
        # Ensure all required fields are present with defaults
        merged_schema = {
            "total_usages": mongodb_schema.get("total_usages", 0) if mongodb_schema else 0,
            "most_used_schema": mongodb_schema.get("most_used_schema") if mongodb_schema else None,
            "most_used_count": mongodb_schema.get("most_used_count", 0) if mongodb_schema else 0,
            "least_used_schema": mongodb_schema.get("least_used_schema") if mongodb_schema else None,
            "least_used_count": mongodb_schema.get("least_used_count", 0) if mongodb_schema else 0,
            "average_usage": mongodb_schema.get("average_usage", 0.0) if mongodb_schema else 0.0,
            "schema_usage": mongodb_schema.get("schema_usage", {}) if mongodb_schema else {},
            "schema_last_used": mongodb_schema.get("schema_last_used", {}) if mongodb_schema else {}
        }
        
        # Get all jobs from MongoDB ONLY
        if mongodb_db.is_connected():
            jobs = mongodb_db.get_jobs(1000, user_id)  # Get last 1000 jobs from MongoDB
        else:
            jobs = []
        ...
        for job in recent_activity:
            # Handle both dict format (MongoDB) and object format
            job_id = job.get("id") if isinstance(job, dict) else getattr(job, "id", "unknown")
            file_name = job.get("file_name") if isinstance(job, dict) else getattr(job, "file_name", "Unknown")
            job_status = job.get("status") if isinstance(job, dict) else getattr(job, "status", "unknown")
            
            # Parse created_at - could be ISO string or timestamp
            created_at = job.get("created_at") if isinstance(job, dict) else getattr(job, "created_at", None)
            if isinstance(created_at, str):
                try:
                    from dateutil import parser
                    timestamp = parser.parse(created_at).isoformat()
                except:
                    timestamp = created_at
            elif created_at:
                timestamp = datetime.fromtimestamp(created_at).isoformat() if isinstance(created_at, (int, float)) else str(created_at)
            else:
